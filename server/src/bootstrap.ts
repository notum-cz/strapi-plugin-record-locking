...
const bootstrap = ({ strapi }: { strapi: Core.Strapi }) => {
  ...
  if (include && exclude) {
    console.warn(
      'Both include and exclude cannot be used together for record-locking, ignoring exclude configuration.'
    );
  }
  ...
  io.on('connection', (socket) => {
    socket.on('openEntity', async ({ entityDocumentId, entityId }) => {
      try {
        ...
        const userHasAdequatePermissions = await doesUserHaveAdequatePermissions(
    ...
    socket.on('takeoverEntity', async ({ entityId, entityDocumentId }, callback) => {
      try {
        ...
        if (userHasAdequatePermissions) {
          const existingLockRecords = await strapi.db
          ...
          const user = await strapi.db
            .query('admin::user')
            .findOne({ where: { id: String(userId) } });
          for (const record of existingLockRecords) {
            if (record.connectionId !== socket.id) {
              io.to(record.connectionId).emit('takeoverEntityPerformed', {
                entityId,
                entityDocumentId,
